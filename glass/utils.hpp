...
struct running_stats_printer_t {
    ...
    void refresh(std::size_t step = 10000) {
        ...
        if (new_progress - last_printed_progress < step) return;
    ...
    void print(std::size_t progress, std::size_t total) {
        ...
        std::printf("\r%3.3f%% [%.*s%*s] %.0f vectors/s, finished %zu/%zu, %zum%zus", percentage * 100.f, left_pad,
                    bars_k, right_pad, "", vectors_per_second, progress, total, rem_time / 60, rem_time % 60);
        std::fflush(stdout);
