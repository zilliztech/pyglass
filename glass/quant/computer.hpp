...
concept ComputerBaseConcept = requires(Computer computer, int32_t u,
                                       int32_t lines) {
...
template <typename Computer>
concept SymComputerConcept = ComputerBaseConcept<Computer> &&
...
struct ComputerImpl : Computer<Storage> {
  ...
  GLASS_INLINE dist_type operator()(const Y *p) const {
    ...
    return std::apply(
        [&](auto &&...args) {
          return dist_func(q, p, this->tensor.dim_align(), args...);
        },
        args);
  }

  GLASS_INLINE dist_type operator()(int32_t u) const {
    return operator()((const Y *)this->tensor.get(u));
  }
...
struct SymComputerImpl : Computer<Storage> {
  ...
  GLASS_INLINE dist_type operator()(const X *x, const X *y) const {
    return std::apply(
        [&](auto &&...args) {
          return dist_func(x, y, this->tensor.dim_align(), args...);
        },
        args);
  }

  GLASS_INLINE dist_type operator()(int32_t u, int32_t v) const {
    return operator()((const X *)this->tensor.get(u),
