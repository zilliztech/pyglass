...
template <typename Quant>
concept QuantConcept = QuantBaseConcept<Quant> && requires(Quant quant, const float *x, int32_t n) {
    { quant.train(x, n) };
...
template <typename Quant>
concept SymComputableQuantConcept = QuantConcept<Quant> && requires(Quant quant) {
    { quant.get_sym_computer() } -> std::same_as<typename Quant::SymComputerType>;
};

template <Metric METRIC, int32_t AlignWidth, int32_t NBits>
struct Quantizer {
