...
struct SQ8QuantizerUniform : Template {
    ...
    using CalibratorType = std::conditional_t<metric == Metric::L2, AffineCalibrator<127>, SymCalibrator<127>>;
...
#pragma omp parallel for schedule(dynamic)
    ...
    constexpr static auto dist_func = metric == Metric::L2 ? helpa::l2_s7_s7 : helpa::dot_s8_s8;

    constexpr static auto dist_func_sym = dist_func;

    using ComputerType = ComputerImpl<Tensor, dist_func, int32_t, float, int8_t, int8_t>;
    using SymComputerType = SymComputerImpl<Tensor, dist_func_sym, int32_t, int8_t>;
