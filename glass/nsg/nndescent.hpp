...
template <SymComputerConcept ComputerType> struct NNDescent {
  ...
  NNDescent(int32_t nb, const ComputerType &computer)
      : nb(nb), computer(computer) {}

  void Build(int K) {
    printf(
        "Start building NN-Descent with params K=%d S=%d R=%d L=%d niter=%d\n",
        K, S, R, L, iters);
    this->K = K;
    this->L = K + 50;
    srand(random_seed);
