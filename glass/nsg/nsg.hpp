...
template <SymComputableQuantConcept QuantType> struct NSG : public Builder {
  int32_t d;
  int R;
  int L;
  int C;
  ...
  explicit NSG(int R = 32, int L = 0, bool use_ssg = true)
      : R(R), L(L ? L : R + 32), C(R + 100), use_ssg(use_ssg) {
    this->GK = 64;
    this->nndescent_S = 10;
...
#pragma omp parallel for schedule(static, 64)
  ...
  void search_on_graph(const auto &computer, int32_t p, const Graph<int> &graph,
                       int ep, auto &pool, std::vector<Node> &fullset) const {
...
#pragma omp parallel for schedule(dynamic)
  ...
  void sync_prune(const auto &computer, int q, auto &pool,
                  std::vector<Node> &fullset, const Graph<int> &knng,
                  Graph<Node> &graph) {
    ...
    while ((int)result.size() < R && (++start) < (int)fullset.size() &&
           start < C) {
  ...
  void add_reverse_links(const auto &computer, int q,
                         std::vector<std::mutex> &locks, Graph<Node> &graph) {
    for (int i = 0; i < R; i++) {
  ...
  int attach_unlinked(Graph<int32_t> &graph, const auto &computer, auto &vis,
...
create_nsg(const std::string &quantizer = "SQ4U", int32_t R = 32, int32_t L = 0,
           bool use_ssg = true) {
  ...
  if (qua == QuantizerType::SQ8U) {
    return std::make_unique<NSG<SQ8QuantizerUniform<Metric::L2>>>(R, L,
                                                                  use_ssg);
  }
  if (qua == QuantizerType::SQ4U) {
    return std::make_unique<NSG<SQ4QuantizerUniform<Metric::L2>>>(R, L,
                                                                  use_ssg);
  }
  if (qua == QuantizerType::SQ2U) {
    return std::make_unique<NSG<SQ2QuantizerUniform<Metric::L2>>>(R, L,
                                                                  use_ssg);
  }
